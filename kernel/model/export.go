...
// Self-host fork: Export2Liandi (one-click publish to b3log Liandi forum) is removed.
// The upstream route /api/export/export2Liandi and its handler are also gone.
...
func ExportStdMarkdown(id string, assetsDestSpace2Underscore, fillCSSVar, adjustHeadingLevel, imgTag bool) string {
	...
	tree := prepareExportTree(bt)
	// Self-host fork: cloud assets base URL is removed. Local assets are referenced via
